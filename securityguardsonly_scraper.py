"""
Security Guards Only Scraper for Security Leads Automation

This module implements a scraper for Security Guards Only, extracting security job postings.
"""
...
class SecurityguardsonlyScraper(BaseScraper):
    """Scraper for Security Guards Only website."""
    
    def __init__(self, config, logger):
        """Initialize the Security Guards Only scraper.
        ...
        self.base_url = config.get("url", "https://www.securityguardsonly.com/")
        self.jobs_url = "https://www.securityguardsonly.com/jobs/"
    
    def scrape(self, user_agent=None):
        """Scrape Security Guards Only for security job postings.
        ...
        """
        self.logger.info(f"Scraping Security Guards Only: {self.jobs_url}")
        ...
        try:
            ...
            if not listings:
                ...
                listings = soup.select("div.job, article.job, div.job-card, div.job-item")
            
            self.logger.info(f"Found {len(listings)} listings on Security Guards Only")
            ...
            for listing in listings[:10]:  # Limit to first 10 for testing
                try:
                    ...
                    salary_elem = listing.find("div", class_=lambda c: c and "salary" in c.lower()) or listing.find("span", class_=lambda c: c and "salary" in c.lower())
                    ...
                    if detail_url:
                        try:
                            ...
                            req_elem = detail_soup.find("div", class_=lambda c: c and "requirements" in c.lower()) or detail_soup.find("div", class_=lambda c: c and "qualifications" in c.lower())
            ...
            self.logger.error(f"Error scraping Security Guards Only: {str(e)}")
            return []
    ...
    def extract_lead_data(self, item):
        ...
        all_text = f"{item.get('title', '')} {item.get('company', '')} {description} {requirements} {contact_info}"
